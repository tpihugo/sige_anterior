...
class requisicion {
    ...
    function consultaMaterialAlmacen() {
        ...
        foreach ($resultado as $key => $value) {
            echo '
                <option value="'. $value['idMaterial'] .'">'. $value['idMaterial'] .' - '. $value['descripcion'] .'</option>';
        }
    }
    ...
    function consultaMaterialConExistencia() {
        ...
        $query = $pdo->prepare('SELECT idMaterial, descripcion, existencia FROM MATERIAL WHERE '
                . 'tipo = "'.$this->getTipo().'" AND enUso = "Activo" AND existencia > 0');
        $query->execute();

        $resultado = $query->fetchAll();
        foreach ($resultado as $key => $value) {
            echo '
                <option value="'. $value['idMaterial'] .'">'. $value['idMaterial'] .' - '. $value['descripcion'] .' - Existencia: '. $value['existencia'] .'</option>';
        }
    }
    ...
    function requisicionConsulta() {
        ...
        foreach ($resultado as $key => $value) {
            ...
            echo '
                <tr>
                    <td>'. $value['idRequisicion'] .'</td>
                    <td>'. $value['folio'] .'</td>    
                    <td>'. $value['fechaRequisicion'] .'</td>
                    <td>'. $value['tipo'] .'</td>
                    <td>'. $value['responsableAlmacen'] .'</td>
                    <td>'. $value['nombre'] .'</td>   
                    <td>'. $value['area'] .'</td>   
                    <td>'. $value['piso'] .'</td>   
                    <td>'. $value['edificio'] .'</td>   
                    <td><a href="requisicionDetalle.php?id='. $value['idRequisicion'] .'"'
                        . ' class="btn btn-default" target="_blank" role="button">Ver Detalle</a></td>   
                    '.$fila.' 
                </tr>
            ';
        }       
...
//        ejemplo de consulta con arreglo multidimensional
    ...
    function salidaMaterialSurtirEntregado() {
        $pdo = new Conexion();
        $query = $pdo->prepare("UPDATE SALIDA_MATERIAL"
                . " SET estado = :estado WHERE idRequisicion = :idRequisicion AND idMaterial = :idMaterial;");
        $query->bindValue(':estado', 'Surtido');
        $query->bindValue(':idRequisicion', $this->getIdRequisicion());
        $query->bindValue(':idMaterial', $this->getIdMaterial());
        $query->execute();
    }
...
AND sm.idMaterial = '. $this->getIdMaterial() .''
               . ' AND r.fechaRequisicion LIKE "%'.$this->getFechaRequisicion().'%" '
